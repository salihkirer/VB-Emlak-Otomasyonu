...
Public Class isyeriKayit
    ...
    Private Sub btnKaydet_Click(sender As Object, e As EventArgs) Handles btnKaydet.Click
        ...
        sqlCalistir("insert into isyerleri (kayTarihi,turu,sinifi,katSayisi,isinmaSekli,cephe,buyuklugu,odaSayisi,diger,apAdi,binaNo,cadde,semt,mahalle,ilce,il,fiyat,sahibi,acikAdres,fotoYol,durumu) values ('" & DateTime.Now & "','" & cbTuru.Text & "','" & cbSinifi.Text & "','" & cbKatSayisi.Text & "','" & cbIsinma.Text & "','" & cbCephe.Text & "','" & txtBuyuklugu.Text & "','" & cbOdaSayisi.Text & "','" & txtDiger.Text & "','" & txtApAdi.Text & "','" & txtBinaNo.Text & "','" & txtCadde.Text & "','" & txtSemt.Text & "','" & txtMahalle.Text & "','" & cbILce.Text & "','" & cbIL.Text & "','" & txtFiyat.Text & "','" & txtSahibi.Text & "','" & txtAcikAdres.Text & "','" & dosyaAdi & "','" & "Boşta" & "')")
    ...
    Private Sub btnGuncelle_Click(sender As Object, e As EventArgs) Handles btnGuncelle.Click
        ...
        Dim dosyaAdi As String = String.Format("{0:yyyyMMddHHmmss}", Now) & uzanti
        pbFoto.Image.Save(Application.StartupPath & "/Resimler/" & dosyaAdi)
        If MsgBox("İşyeri Bilgileri Güncellenecektir! Onaylıyor musunuz?", vbYesNo, "İşyeri Kaydı Güncelleme") = MsgBoxResult.No Then
            Return
        End If
        sqlCalistir("update isyerleri set turu='" & cbTuru.Text & "',sinifi='" & cbSinifi.Text & "',katSayisi='" & cbKatSayisi.Text & "',isinmaSekli='" & cbIsinma.Text & "',cephe='" & cbCephe.Text & "',odaSayisi='" & cbOdaSayisi.Text & "',buyuklugu='" & txtBuyuklugu.Text & "',fiyat='" & txtFiyat.Text & "',sahibi='" & txtSahibi.Text & "',diger='" & txtDiger.Text & "',apAdi='" & txtApAdi.Text & "',binaNo='" & txtBinaNo.Text & "',cadde='" & txtCadde.Text & "',semt='" & txtSemt.Text & "',mahalle='" & txtMahalle.Text & "',ilce='" & cbILce.Text & "',il='" & cbIL.Text & "',acikAdres='" & txtAcikAdres.Text & "',fotoYol='" & dosyaAdi & "' where gmKodu=" & Convert.ToDouble(txtGmKodu.Text))
        MsgBox("İşyeri Güncellendi", MsgBoxStyle.Information, "İşlem Tamam")
        MDIParent1.isyeriGetir()
        Me.Close()
    End Sub
End Class